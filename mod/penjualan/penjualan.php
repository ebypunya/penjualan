							...
							while ($m = mysql_fetch_assoc($sql_kul)) {
								echo"<tr>
									...
									<td><form action='$aksi'>
										...
										<div class='w3-row'>
											<div class='w3-col s4'><input type='text' name='qty' value='$tmp[qty]' class='w3-input w3-tiny w3-border' maxlength='3' placeholder=''></div>
						...
						if(mysql_num_rows($sql_tmp) > 0)
						{
							while ($b = mysql_fetch_assoc($sql_tmp)) {
								...
								echo"<tr style='border-bottom:1px dashed #ccc;'>
									...
									<td><center>".number_format($b['qty'])."</center></td>
			...
			if(isset($_GET['id']))
			{
				...
				echo"<table class='w3-table w3-tiny'>
					...
					<tr style='border-bottom:1px dashed #ccc;'>
						<td>Nama / Email</td>
						<td>:</td>
						<td><b>$tra[nama_pelanggan] / "?><?php echo !empty($tra['email_pelanggan']) ? $tra['email_pelanggan'] : "-"; ?><?php echo"</b></td>
					</tr>
