		...
		case 'form':
			...
			echo"<form class='w3-small' method='POST' action='$act'>
				...
				<table>
					...
					<tr>
						<td><label class='w3-label'>NILAI (MARGIN)</label></td>
						<td>:</td>
						<td><input type='text' name='margin' class='w3-input mny' value='"?><?php echo isset($c['harga_jual']) - ($c['harga_beli']) ? $c['harga_jual'] - ($c['harga_beli']) : '';?><?php echo"' >
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td><label class='w3-label'>Supplier</label></td>
						<td>:</td>
						<td><div class='w3-row'>
							<div class='w3-col s4'>
								<select name='kode_supplier' class='w3-select'>";
								$sql_supplier = mysql_query("SELECT * FROM tb_supplier");
								while($ks = mysql_fetch_assoc($sql_supplier))
								{
									if(isset($c['kode_supplier']) && $ks['kode_supplier'] == $c['kode_supplier'])
									{
										echo"<option value='$ks[kode_supplier]' selected>$ks[nama_toko]</option>";	
									}
									else
									{
		...
		default :
			...
			<table class='w3-table w3-striped w3-bordered w3-tiny w3-hoverable tbl'>
				...
				if($fd_kul > 0)
				{
					...
					<td>Rp. ".number_format($m['harga_jual'] - ($m['harga_beli']))."</td>
