<?php
	...
	{
		$sql = mysql_query("SELECT jml FROM tb_detail_pembelian WHERE kode_barang = '$id'");
		$total = 0;
	...
	{
		$sql = mysql_query("SELECT jml FROM tb_detail_penjualan WHERE kode_barang = '$id'");
		$total = 0;
		while ($q = mysql_fetch_assoc($sql)) {
			$total = $total + $q['jml'];
		}
